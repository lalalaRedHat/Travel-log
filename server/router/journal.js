...
//日志评论发布
j.post('/comment', (req, res) => {
  //获取URL地址栏的参数
  let obj = req.body;
  // console.log(obj);
  let sql = "SELECT uid FROM dhz_users WHERE phone=?";
  pool.query(sql, [obj.phone], (err, user_id) => {
    if (err) throw err;
    let uid = user_id[0].uid;
    // console.log(user_id);
    sql = 'INSERT INTO dhz_discuss (comment,time,discuss_id,journal_id) VALUES (?,?,?,?)';
    pool.query(sql, [obj.comment, obj.time, uid, obj.journal_id], (error, result) => {
      if (error) throw error;
      res.send({ code: 1 });
    });
  });
...
//日志评论查询
j.get('/All_comments', (req, res) => {
  let jid = req.query.id;
  // console.log(jid);
  let sql = 'SELECT comment,time,quantity,avatar,nickname FROM dhz_discuss INNER JOIN dhz_users ON discuss_id = uid WHERE journal_id=? ORDER BY did DESC';
  pool.query(sql, [jid], (err, result) => {
    if (err) throw err;
    res.send({ code: 1, comments: result });
  });
});
